...
Meteor.ui._Patcher._copyAttributes = function(tgt, src) {
  ...
  // walk attributes on target, changing or removing
  for(var i=tgtAttrs.length-1;i>=0;i--) { // iterate backwards
    var a = tgtAttrs.item(i);
    if (a.specified) {
      // an attribute on the target -- needs to be
      // replaced or deleted
      var b = src.getAttributeNode(a.name);
      if (b && b.specified) {
        a.value = b.value;
      } else{
        tgt.removeAttributeNode(a);
      }
    }
  }

  // set attributes on target that aren't already present
  for(var i=0; i<srcAttrs.length; i++) {
    var b = srcAttrs.item(i);
    // setting an attribute that already exists seems
    // to create two attributes in IE7, so we're careful
    if (b.specified && ! tgt.getAttributeNode(b.name)) {
      // copy source attribute to target
      tgt.setAttribute(b.name, b.value);
    }
  }
};
