...
(function() {
  ...
  Meteor.ui.chunk = function(html_func, react_data) {
    ...
    if (! live) {
      // XXX This case will be ok on server with server-side rendering
      throw new Error("Meteor.ui.chunk must be called inside Meteor.ui.render");
      //return html_func();
    }
  ...
  Meteor.ui.listChunk = function (observable, doc_func, else_func, react_data) {
    ...
    if (! live) {
      // XXX This case will be ok with server-side rendering
      throw new Error("Meteor.ui.listChunk must be called inside Meteor.ui.render");
    }
