...
test("template assembly", function () {
  ...
  assert.equal(Meteor.ui._canonicalizeHtml(onscreen.innerHTML), "xyhi");
  Session.set("stuff", false);
  Meteor.flush();
  assert.equal(Meteor.ui._canonicalizeHtml(onscreen.innerHTML), "xhi");
...
test("template table assembly", function() {
  ...
  assert.equal(table.rows.length, 3, table.parentNode.innerHTML);
